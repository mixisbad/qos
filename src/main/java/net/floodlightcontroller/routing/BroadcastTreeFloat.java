...
public class BroadcastTreeFloat {
    protected HashMap<Long, Link> links;
    protected HashMap<Long, Float> costs;

    public BroadcastTreeFloat() {
        links = new HashMap<Long, Link>();
        costs = new HashMap<Long, Float>();
    }

    public BroadcastTreeFloat(HashMap<Long, Link> links, HashMap<Long, Float> costs) {
        this.links = links;
        this.costs = costs;
    }
    ...
    public Float getCost(long node) {
        if (costs.get(node) == null) return -1f;
    ...
    public HashMap<Long, Float> getCosts() {
        return costs;
    }
}
