...
def measure_bandwidth():
    ...
    with contextlib.closing(urllib2.urlopen('http://localhost:8080/wm/core/switch/all/flow/json')) as page:
                ...
                for i in range( len(adjacent[switch_index]) ):
                    ...
                    for j in range( len(tmp_cal) ):
                        if total_momentum > 0:
                            ...
                            if queue_property[switch_index][i][j][0] < speed[switch_index][i]/20.0 :
                                queue_property[switch_index][i][j][0] = speed[switch_index][i]/20.0
                        else:
                            #queue_property[switch_index][i][j][0] = 0.0
                            queue_property[switch_index][i][j][0] = speed[switch_index][i]
                            #try set for max
        ...
        i = 0
        j = 0
        tmp_num_sw = len(bandwidthout)
        for sw in bandwidthout:
            f_ptr.write( unicode(str(len(sw)) + " "))
            j = 0
            for port in sw :
                f_ptr.write(unicode( str( round(port[0],4) if port[0] >= 0 else 0)  ))
                if adjacent[i][j] < tmp_num_sw and adjacent[i][j] >= 0:
                    f_ptr.write(unicode( " " + str(adjacent[i][j]) + " "  + str(port_matrix[adjacent[i][j]][i]) + " " ))
                else:
                    f_ptr.write( unicode(" -1 -1 "))
                j+=1
            f_ptr.write(u"\n")
            f_ptr.flush()
            i+=1
    ...
    for index in range(0, len(queue_property) ):
        ...
        for port_num in range(0, len(queue_property[index]) ):
            if focus_dpid in name_index:
                ...
                queuecmd = queuecmd + " -- "
                queuecmd = queuecmd +     "--id=@q0 create queue other-config:max-rate=%d other-config:min-rate=%d " % ( int(qosmax) , int(qosmax/20) )
                for i in range(1, len(queue_property[index][port_num])+1):
...
if __name__ == '__main__':
    ...
    tmp_sw_num = len(switch_nodes)

    adjacent = [[] for i in range( tmp_sw_num )]

    speed = [[] for i in range( tmp_sw_num )]

    queue_property = [[] for i in range( tmp_sw_num )]
    
    port_matrix = [[ -1 for x in range( tmp_sw_num )] for y in range( tmp_sw_num )]
    ...
    while len(item) > 0:
        ...
        for i in range(1,len(item)):
            ...
            if item_info[0] in nodes:
                tmp_index = nodes.index(item_info[0])
                row.append( tmp_index )
                if tmp_index < tmp_sw_num:
                    port_matrix[switch_index][tmp_index] = i
            else:
                row.append( -1 )
