...
def main():
    ...
    parser.add_argument('-p','--port', required=False, default="8080", type=str, dest='p', metavar="P")
    parser.add_argument('-c','--controller', required=False, default="127.0.0.1", dest="c", type=str, metavar="C")
    parser.add_argument('-e','--enable', required=False, dest="qos_op", action="store_const", const="enable")	
    parser.add_argument('-d','--disable', required=False, dest="qos_op", action="store_const", const="disable")
    parser.add_argument('-s','--status', required=False, dest="qos_op", action="store_const", const="status")
    parser.add_argument('-A','--add', required=False, dest="action_op", action="store_const", const="add")
    parser.add_argument('-D','--delete', required=False, dest="action_op", action="store_const", const="delete")
    parser.add_argument('-M','--modify', required=False, dest="action_op", action="store_const", const="modify")
    parser.add_argument('-L','--list', required=False, dest="action_op", action="store_const", const="list")
    parser.add_argument('-t','--type', required=False, dest="type", choices=["policy","service","policies","services"])
    parser.add_argument('-O','--json', required=False, dest="obj")
...
def write_remove(op,u_id):
    ...
    if op == "service":
        ...
        try:
            ...
            cnx = mysql.connector.connect(user='thesis', password = 'password', host='10.0.2.15', database='thesis')
            cursor = cnx.cursor()
            query = "SELECT name FROM policy WHERE name = '%s' " % u_id
            cursor.execute(query)
            row = cursor.fetchone()
            if row is not None:
                found = True
            ...
            if not found:
                ...
                query = "DELETE FROM policy WHERE name = '%s' " % u_id
                cursor.execute(query)
...
class httpHelper:
    ...
    def request(self, method, url, body, content_type="application/json"):
        ...
        if s != ok and s != acc and s != crtd and s != ncontnt:
            print "%s to %s got an unexpected response code: %d %s (content = '%s')" % (method, url, response.status, response.reason, response.read())
