...
for sw in switches:
        ...
        queuecmd = queuecmd + "create qos type=linux-htb other-config:max-rate=20000 queues=0=@q0,1=@q1,2=@q2 -- "
        queuecmd = queuecmd + "--id=@q0 create queue other-config:max-rate=10000 -- "
        queuecmd = queuecmd + "--id=@q1 create queue other-config:max-rate=20000 -- "
        queuecmd = queuecmd + "--id=@q2 create queue other-config:max-rate=20000" 
	print queuecmd
